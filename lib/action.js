...
export const askGemini = async (prompt) => {
  const ai = new GoogleGenAI({
    apiKey: "AIzaSyDUYzJ-7FrNYBRrV37xfSTy7bomWe_5o2E",
  });
  try {
...
//get all customers
export const getAllCustomers = async (adminEmail) => {
  try {
    const response = await axios.get(
      `http://localhost:8181/admin/getAllCustomers/${adminEmail}`
    );
    return response.data;
  } catch (error) {
    console.log(error);
...
export const getAllVendors = async (adminEmail) => {
  try {
    const response = await axios.get(
      `http://localhost:8181/admin/getAllVendors/${adminEmail}`
    );
    return response.data;
  } catch (error) {
    console.log(error);
